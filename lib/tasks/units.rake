namespace :units do
  ...
  task :build_i18n_from_csv => :environment do
    ...
    SpreadsheetFile.parse 'config/units-of-measure/translations_piece.csv', csv_options do |row, index|
      ...
      translation_de = { name: row[4].strip }
      translation_de[:abbreviation] = row[5].strip unless row[5].blank?
    ...
    [:en, :de].each do |locale|
      SpreadsheetFile.parse "config/units-of-measure/translations_scalar-#{locale}.csv", csv_options do |row, index|
        ...
        translation[:symbol] = row[3].strip unless row[3].blank?
        alternate_symbols = row[4]&.strip
        unless alternate_symbols.blank?
          alternate_symbols.split(', ').each_with_index do |symbol, index|
            translation["alternate_symbol_#{index}".to_sym] = symbol.strip
          end
        end
        translations[locale][code] = translation
      end
      result = "# This file has been autogenerated by `rake units:build_i18n_from_csv`\n#{{ "#{locale}": { unece_units: translations[locale] } }.deep_stringify_keys.to_yaml}"
      File.write(File.expand_path("config/locales/unece_#{locale}.yml", Rails.root), result)
    end
  end
end
